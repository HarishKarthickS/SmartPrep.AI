      ...
      setTimeout(() => {
        if (id) {
          const getSaved = async () => {
            let res = null;
            try {
              res = await instance.get("http://localhost:5000/api/chat/saved", {
                params: { chatId: id },
              });
            } catch (err) {
              console.log(err);
...
const InputArea = ({ status, chatRef, stateAction }) => {
  ...
  const FormHandle = async () => {
    if (prompt?.length > 0) {
      ...
      try {
        if (_id) {
          res = await instance.put("http://localhost:5000/api/chat", {
            chatId: _id,
            prompt,
          });
        } else {
          res = await instance.post("http://localhost:5000/api/chat", { prompt });
        }
      } catch (err) {
        console.log(err);
  ...
  // Inline styles for the textarea (input bar)
  const textAreaStyle = {
    height:"39px",
  ...
  return (
    <div className="inputArea">
      ...
      <div className="text">
        ...
        >
          Smartprep V 1 Version.
        </a>{" "}
        Free Research Preview. Our goal is to make AI systems more natural and safe
