...
const admin = require("firebase-admin"); // Added to access Firestore for the user gist
...
router.post("/", firebaseAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    const chosenModel = model || "qwen/qwen-vl-plus:free";
...
if (userSnapshot.exists && userSnapshot.data().gist) {
  // Prepend a system message indicating that the bot is a teaching assistant.
  messages.unshift({
    role: "system",
  ...
  // If no gist exists, you can optionally include a default teaching context.
  messages.unshift({
    role: "system",
    ...
    try {
      const completion = await openai.chat.completions.create({
        model: chosenModel,
        messages, // Send the (possibly trimmed) conversation context including the system message
      });
    ...
    try {
      if (chatId) {
        await chat.updateChat(chatId, prompt, { openai: openaiResponse }, req.user.userId);
        responseData.chatId = chatId;
      } else {
        const result = await chat.newResponse(prompt, { openai: openaiResponse }, req.user.userId);
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        context: messages, // Return the conversation context
      },
    });
  } catch (err) {
